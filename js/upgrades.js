...
// Update the upgrade panel with tabbed interface for Command Mode
function updateSelectionUpgradePanel() {
    if (gameState.currentMode !== 'command') return;
    
    const upgradePanel = document.getElementById('upgradePanel');
    
    // Hide traditional upgrade table for Command Mode
    const upgradeTable = document.querySelector('.upgrade-table');
    if (upgradeTable) {
        upgradeTable.style.display = 'none';
    }
    
    // Get or create tabbed upgrade panel
    let tabbedPanel = document.getElementById('tabbedUpgradePanel');
    if (!tabbedPanel) {
        tabbedPanel = document.createElement('div');
        tabbedPanel.id = 'tabbedUpgradePanel';
        tabbedPanel.style.cssText = `
            margin-top: 20px;
            background: rgba(0, 255, 0, 0.1);
            border: 1px solid #0f0;
            border-radius: 5px;
        `;
        upgradePanel.appendChild(tabbedPanel);
...
// Create tabbed upgrade interface
function updateTabbedUpgradePanel() {
    ...
    let html = `
        <div style="display: flex; border-bottom: 1px solid #0f0;">
            <button onclick="switchUpgradeTab('global')" 
                    style="flex: 1; padding: 10px; background: ${currentTab === 'global' ? '#0f0' : 'transparent'}; 
                           color: ${currentTab === 'global' ? '#000' : '#0f0'}; border: none; cursor: pointer;">
                GLOBAL
            </button>
            <button onclick="switchUpgradeTab('turrets')" 
                    style="flex: 1; padding: 10px; background: ${currentTab === 'turrets' ? '#0f0' : 'transparent'}; 
                           color: ${currentTab === 'turrets' ? '#000' : '#0f0'}; border: none; cursor: pointer;">
                TURRETS
            </button>
            <button onclick="switchUpgradeTab('cities')" 
                    style="flex: 1; padding: 10px; background: ${currentTab === 'cities' ? '#0f0' : 'transparent'}; 
                           color: ${currentTab === 'cities' ? '#000' : '#0f0'}; border: none; cursor: pointer;">
                CITIES
            </button>
        </div>
    ...
    if (currentTab === 'global') {
        html += getGlobalUpgradesHTML();
    } else if (currentTab === 'turrets') {
        html += getTurretsUpgradesHTML();
    } else if (currentTab === 'cities') {
        html += getCitiesUpgradesHTML();
    }
...
function getGlobalUpgradesHTML() {
    let html = `
        <h4 style="color: #0f0; margin-top: 0;">Global Upgrades</h4>
    ...
    if (!researchUnlocked) {
        html += `
            <button onclick="purchaseGlobalUpgrade('research')" 
                    style="padding: 10px; background: ${canAffordResearch ? 'rgba(0, 255, 0, 0.2)' : 'rgba(128, 128, 128, 0.2)'};
                           color: ${canAffordResearch ? '#0f0' : '#888'}; border: 1px solid ${canAffordResearch ? '#0f0' : '#888'};
                           cursor: ${canAffordResearch ? 'pointer' : 'not-allowed'}; border-radius: 3px;"
                    ${!canAffordResearch ? 'disabled' : ''}>
                <strong>Unlock Science</strong><br>
                <small>Enable science production</small><br>
                <small>${researchCost} scrap</small>
            </button>
        `;
    } else {
        html += `
            <div style="padding: 10px; background: rgba(0, 255, 0, 0.1); border: 1px solid #0f0; border-radius: 3px; text-align: center;">
                <strong style="color: #0f0;">Science Unlocked</strong><br>
                <small>Cities can now produce science</small>
            </div>
        `;
    }
    
    // Other global upgrades
    const globalUpgradesList = [
    ...
    globalUpgradesList.forEach(upgrade => {
        ...
        if (!isOwned) {
            html += `
                <button onclick="purchaseGlobalUpgrade('${upgrade.id}')" 
                        style="padding: 10px; background: ${canAfford ? 'rgba(0, 255, 0, 0.2)' : 'rgba(128, 128, 128, 0.2)'};
                               color: ${canAfford ? '#0f0' : '#888'}; border: 1px solid ${canAfford ? '#0f0' : '#888'};
        ...
        html += `
            <div style="padding: 10px; background: rgba(0, 255, 255, 0.1); border: 1px solid #0ff; border-radius: 3px;">
                ...
                <button onclick="selectEntity('turret', ${index})" 
                        style="width: 100%; padding: 8px; background: rgba(0, 255, 255, 0.2); color: #0ff; 
                               border: 1px solid #0ff; border-radius: 3px; cursor: pointer;">
                    Select Turret
                </button>
            </div>
        `;
        ...
        turretUpgrades.forEach(upgrade => {
            ...
            html += `
                <button onclick="upgrade('${upgrade.id}', ${turretIndex})" 
                        style="padding: 10px; background: ${canAfford ? 'rgba(0, 255, 255, 0.2)' : 'rgba(128, 128, 128, 0.2)'};
                               color: ${canAfford ? '#0ff' : '#888'}; border: 1px solid ${canAfford ? '#0ff' : '#888'};
                               cursor: ${canAfford ? 'pointer' : 'not-allowed'}; border-radius: 3px;"
                        ${!canAfford ? 'disabled' : ''}>
                    <strong>${upgrade.name}</strong><br>
        ...
        if (destroyedCities.includes(index)) {
            html += `
                <div style="padding: 10px; background: rgba(255, 0, 0, 0.1); border: 1px solid #f00; border-radius: 3px;">
                    ...
                    <button onclick="selectEntity('city', ${index})" 
                            style="width: 100%; padding: 8px; background: rgba(255, 0, 0, 0.2); color: #f00; 
                                   border: 1px solid #f00; border-radius: 3px; cursor: pointer;">
                        Rebuild (Coming Soon)
                    </button>
                </div>
            `;
            ...
            html += `
                <div style="padding: 10px; background: rgba(0, 255, 0, 0.1); border: 1px solid #0f0; border-radius: 3px;">
                    ...
                    <button onclick="selectEntity('city', ${index})" 
                            style="width: 100%; padding: 8px; background: rgba(0, 255, 0, 0.2); color: #0f0; 
            ...
            modes.forEach(mode => {
                ...
                html += `
                    <button onclick="setCityProductionMode(${cityIndex}, '${mode.id}')" 
                            style="flex: 1; padding: 10px; background: ${isSelected ? mode.color : 'transparent'};
                                   color: ${isSelected ? '#000' : mode.color}; border: 1px solid ${mode.color};
                                   cursor: pointer; border-radius: 3px; opacity: ${isSelected ? '1' : '0.7'};"
...
function purchaseGlobalUpgrade(upgradeId) {
...
function setCityProductionMode(cityIndex, mode) {
    if (gameState.currentMode !== 'command') return;
...
function upgradeCityFeature(cityIndex, feature) {
    if (gameState.currentMode !== 'command') return;
